...
# Find everything from "<!-- Production Experience -->" up to "<!-- AI Optimization / FAQ Semantic Block -->"
# We will replace that whole chunk.
match = re.search(r'<!-- Production Experience -->.*?(?=<!-- AI Optimization / FAQ Semantic Block -->)', content, flags=re.DOTALL)
if match:
    old_middle = match.group(0)
    
    # We also need to extract the 4 team cards from old_middle before replacing
    team_cards_match = re.search(r'<!-- Team Member 1 -->.*?</div>\s*</div>\s*</section>\s*$', old_middle, flags=re.DOTALL)
    
    if team_cards_match:
        team_cards = team_cards_match.group(0)
        # Update classes on team cards for hover effects
        team_cards = team_cards.replace('class="card"', 'class="team-card"')
        
        # Remove the closing tags from team_cards as they will be added by new_sections
        # The team_cards match ends with </div></div></section>
        # We just want the cards themselves
        cards_only_match = re.search(r'<!-- Team Member 1 -->.*?(?=</div>\s*</div>\s*</section>)', team_cards, flags=re.DOTALL)
        
        if cards_only_match:
            cards_only = cards_only_match.group(0)
            final_replacement = new_sections + cards_only + "\n</div>\n</div>\n</section>\n"
            content = content.replace(old_middle, final_replacement)
            
            with open("about/index.html", "w") as f:
                f.write(content)
            print("Successfully updated About page.")
        else:
            print("Failed to isolate team cards.")
    else:
        print("Failed to find team cards.")
else:
    print("Failed to match the middle section.")
