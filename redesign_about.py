...
# Extract the team cards exactly as they are currently
team_cards_match = re.search(r'<!-- Team Member 1 -->.*?(?=</div>\s*</div>\s*</section>)', content, flags=re.DOTALL)
if team_cards_match:
    team_cards = team_cards_match.group(0)
    # Inject the new team-card class
    team_cards = team_cards.replace('class="card"', 'class="team-card"')
    new_sections += "\n" + team_cards + "\n</div>\n</div>\n</section>\n"
else:
    print("Could not find team cards block!")

# Replace everything from <!-- Production Experience --> to the end of <!-- Meet the Team -->
content = re.sub(r'<!-- Production Experience -->.*?</section>\s*</div>\s*</div>\s*</section>', new_sections, content, flags=re.DOTALL)

with open("about/index.html", "w") as f:
    f.write(content)

print("About page successfully redesigned.")
